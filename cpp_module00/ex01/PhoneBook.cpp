...
/*   Updated: 2022/12/11 21:18:47 by ayassir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_print_index(PhoneBook one)
{
    ...
    while (getline(std::cin, index))
    {
        ...
        if (index.empty() || index.size() > 2 || !isdigit(index.c_str()[0]))
        {
        ...
        else if (!index.empty())
        {
            ...
            {
                std::cout << "Index is empty, try another one" << std::endl;
                return ;
            }
...
PhoneBook    ft_phonebook_add(PhoneBook   Phonebook)
{
    ...
    {
        if (str.empty())
            std::cout << "Enter the getnickName() : "; 
        else if (!str.empty())
        {
    ...
    std::cout << "Contact " << index + 1 << " was added successfully" << std::endl;
    index++;
     if (index == 8)
        index = 0;
    return (Phonebook);
